...
import com.spring.example.VO.KakaoLoginVO;
...
public class HomeController {
	...
	public String redirect(@RequestParam String code, HttpSession session) throws IOException {
		...
		KakaoLoginVO output = mapper.readValue(out, KakaoLoginVO.class);
		
		System.out.println(output);
		//session.setAttribute("id", output.getAccess_token());
		session.setAttribute("id", output.getScope());
		//session.setAttribute("email", output.getScope());
		return "redirect:/";
	}
